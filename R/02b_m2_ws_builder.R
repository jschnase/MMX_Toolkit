...
for (year in seq(start_yr, stop_yr, step_yr)) {
    ...
    for (var in files) {
        ...
        # lores_m      <- mask(raster(lores_c), resample(mask, raster(lores_c), method='bilinear'))
        ...
        hires_m      <- mask(raster(hires_c), resample(mask, raster(hires_c), method='bilinear'))
