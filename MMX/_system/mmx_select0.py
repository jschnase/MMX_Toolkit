...
#   (run from the CLI from MTX_MMX_DIRECTORY, directory path is backed in ...)
#
# ==============================================================================


# import libraries
import os, sys, shutil, random, csv, datetime, time, numpy as np

# set default system working directories
# mmx_home                        = '/Users/jschnase/Dropbox/MMX-Project/MMX'
# mmx_home                        = '/Users/jschnase/MMX'
# mmx_home                        = '/home/jschnase/MMX/mmx_jls_v1.0/MMX'
mmx_home                        = '/Users/jschnase/Dropbox/MMX-Project/MMX-Development/mmxToolKit/MMX'

# set user's working directory from input argument or set to system default
if len(sys.argv) > 1 :
    working_directory           = sys.argv[1]  
else :
    working_directory           = mmx_home
...
mmx_selection_set               = os.path.join(working_directory, 'selection_set')

working_set_directory           = os.path.join(working_directory, 'working_set')
working_set_list                = os.listdir(working_set_directory)
working_set_count               = len(working_set_list)
# -------------------------------

print(">>> mmx_select0.py")
...
top_ten_file = os.path.join(mmx_selection_set, mmx_uid + '-top_ten.csv')
...
with open(top_ten_file, 'w') as txt_file :
    ...
    while count > limit :
        ...
        dst = mmx_selection_set + '/' + working_set_list[int(avg_sorted_tally_table[count][0])] + '.asc'
        shutil.copy(src,dst)
        
        count = count - 1

print(">>> mmx_select0.py done")
